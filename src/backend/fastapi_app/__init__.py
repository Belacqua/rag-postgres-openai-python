...
from fastapi_app.openai_clients import create_anthropic_chat_client, create_openai_embed_client
...
class State(TypedDict):
    ...
    chat_client: anthropic.AsyncAnthropic
...
async def lifespan(app: fastapi.FastAPI) -> AsyncIterator[State]:
    ...
    if os.getenv("OPENAI_EMBED_HOST") == "azure" or os.getenv("POSTGRES_HOST", "").endswith(".database.azure.com"):
    ...
    chat_client = await create_anthropic_chat_client()
